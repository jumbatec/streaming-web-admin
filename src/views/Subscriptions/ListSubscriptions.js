...
class ListSubscriptions extends Component {
  ...
  loadSubscriptions(page) {
    ...
    if (this.state.lastdata.length > 0) {
      const dataToPaginate = this.state.filteredData.length > 0 ? this.state.filteredData : this.state.lastdata;
  ...
  render() {
    const { selectedYear, selectedMonth, selectedStatus, startDate, endDate, years, statusOptions, showPrintModal } = this.state;
      ...
      <div className="animated fadeIn">
                  ...
                  <Row>
                    ...
                    <Col md={2}>
                      ...
                      >
                        ...
                        {months.map(month => (
                          <option key={month.code} value={month.code}>{month.desc}</option>
                        ))}
                      </CFormSelect>
                    </Col>
                    <Col md={2}>
        ...
        >
          ...
          <CModalBody>
            ...
            <ul>
              <li><strong>Ano:</strong> {selectedYear}</li>
              <li><strong>Mês:</strong> {selectedMonth ? months.find(m => m.code === selectedMonth)?.desc : 'Todos'}</li>
              <li><strong>Estado:</strong> {selectedStatus ? statusOptions.find(s => s.value === selectedStatus)?.label : 'Todos'}</li>
              <li><strong>Data Início:</strong> {startDate ? startDate.toLocaleDateString('pt-BR') : 'Não definido'}</li>
              <li><strong>Data Fim:</strong> {endDate ? endDate.toLocaleDateString('pt-BR') : 'Não definido'}</li>
              <li><strong>Total de Subscrições:</strong> {this.state.filteredData.length}</li>
            </ul>
          </CModalBody>
          <CModalFooter>
